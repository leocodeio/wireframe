...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    localStorage.setItem('sr_authed', role); // 'user' or 'admin'
  ...
  const roleConfig = {
    user: {
      label: 'Traveller',
      subtitle: 'Sign in to your SafeRoam account',
      accent: '#00694C',
      accentBg: '#F0FBF5',
      icon: 'luggage',
    },
    ...
    <div className="flex flex-col items-center w-full min-h-screen bg-[#FCF9F5]">
      <div className="w-full max-w-lg relative flex flex-col min-h-screen shadow-2xl bg-surface mx-auto">
        ...
        <div className="flex items-center justify-center gap-2 pt-6 px-6">
          {(['user', 'admin'] as Role[]).map(r => (
            <button
              key={r}
              onClick={() => setRole(r)}
              className={`flex-1 py-2.5 rounded-full text-[13px] font-bold border transition-all active:scale-95 flex items-center justify-center gap-1.5 ${
                role === r
                  ? r === 'user'
            ...
            >
              <span
                ...
                style={role === r ? { fontVariationSettings: "'FILL' 1" } : {}}
              >
                {r === 'user' ? 'person' : 'admin_panel_settings'}
              </span>
              {r === 'user' ? 'Traveller' : 'Admin / Agent'}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-col items-center pt-10 pb-8 px-6">
          {/* Logo badge */}
          <div
            className="w-20 h-20 rounded-2xl flex items-center justify-center shadow-md mb-5 transition-all duration-300"
            style={{ background: cfg.accentBg }}
          >
            <span
              className="material-symbols-outlined text-[42px] transition-all duration-300"
              style={{ color: cfg.accent, fontVariationSettings: "'FILL' 1" }}
            >
              {cfg.icon}
            </span>
          </div>
          ...
          <p className="text-on-surface-variant text-[14px] mt-1 text-center">{cfg.subtitle}</p>
        </div>
        ...
        <div className="flex-1 px-6 pb-10">
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[13px] font-semibold text-on-surface-variant uppercase tracking-wide" htmlFor="email">
                {role === 'admin' ? 'Admin Email' : 'Email'}
              </label>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-[18px] text-on-surface-variant">
                  mail
                </span>
                <input
                  id="email"
                  type="email"
                  required
                  placeholder={role === 'admin' ? 'admin@saferoam.app' : 'you@example.com'}
                  value={email}
            ...
            {/* Password */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[13px] font-semibold text-on-surface-variant uppercase tracking-wide" htmlFor="password">
                Password
              </label>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 material-symbols-outlined text-[18px] text-on-surface-variant">
            ...
            {role === 'admin' && (
              <div className="flex items-start gap-2.5 bg-[#F0F4FF] border border-[#1565C0]/20 rounded-xl px-4 py-3">
                <span className="material-symbols-outlined text-[18px] text-[#1565C0] shrink-0 mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>
                  info
                </span>
                <p className="text-[12px] text-[#1565C0] leading-relaxed">
                  Admin access is restricted to authorised SafeRoam personnel and licensed travel agents.
                </p>
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 rounded-xl font-bold text-[16px] text-white flex items-center justify-center gap-2 active:scale-95 transition-all shadow-md mt-2 disabled:opacity-70"
